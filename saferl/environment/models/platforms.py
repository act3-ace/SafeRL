        ...
        for i, actuator_config in enumerate(self.actuator_config_list):
            ...
            if actuator.space == 'continuous':
                ...
                if ('space' not in actuator_config) or (actuator_config['space'] == 'continuous'):
                    ...
                    cont_act_config_keys = ['name', 'space', 'bounds', 'rescale', 'zero_centered', 'pre_activation']
                    ...
                    pre_activation = actuator_config.get('pre_activation', "clip")
                    if ('rescale' not in actuator_config) or (actuator_config['rescale']):
                        ...
                        assert not pre_activation, \
                            f"pre_activation not supported without rescale for actuator {actuator.name}"
...
class ActionPreprocessorContinuousRescale(ActionPreprocessor):
    def __init__(self, name, bounds, zero_centered=False, pre_activation="clip"):
        self.bounds = bounds
        self.zero_centered = zero_centered

        if pre_activation == "clip":
            self.pre_activation = lambda x: np.clip(x, -1, 1)
        elif pre_activation == "tanh":
            self.pre_activation = np.tanh
        else:
            raise ValueError("Invalid value for pre_activation. Must be [None, 'tanh']")
    ...
    def preprocess(self, action):
        ...
        old_action = action
        action = self.pre_activation(action)
